...
namespace HttpConnect
{
	...
	private void CopyStream(NetworkStream src, NetworkStream dst)
	{
		...
		try {
		    ...
		    // close or error
		    Console.WriteLine("CopyStream {0}", e);
		    return;
		}
	    }
	}
	...
	public async Task<int> RelayTask(TcpClient cli)
	{
		...
		Console.WriteLine("Trying to connect {0} {1}", addrs[0], port);
		var sock = new TcpClient();

		await sock.ConnectAsync(ep.Address, ep.Port);
		Console.WriteLine("connected to proxy {0} {1}", addrs[0], port);
		...
		using (NetworkStream stream = sock.GetStream()) {
		    ...
		    Console.WriteLine("proxy response code={0}", resp_a[1]);
		    if (resp_a[1] != "200") {
			// bad resp
			return 0;
		    }
		...
		Console.WriteLine("Relay Exception {0}", e);
	    }
	...
	static void Main(string[] args)
	{
		...
		Console.WriteLine("Bad target: {0}", target);
		return;
	    }
